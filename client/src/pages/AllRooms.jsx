  ...
  return (
    <label className="flex gap-3 items-center cursor-pointer mt-2 text-sm">
      <input
        type="checkbox"
        checked={selected}
        onChange={(e) => onChange(e, e.target.checked, label)}
      />
...
function AllRooms() {
  ...
  const priceRange = ['0 to 500', '500 to 1000', '1000 to 2000', '2000 to 3000', '10000 to 20000'];
  const sortOptions = ['Price Low to High', 'Price High to Low', 'Newest First'];

  const handleCheckboxChange = (e, checked, label, stateSetter, currentState) => {
    if (checked) {
      stateSetter([...currentState, label]);
    } else {
      stateSetter(currentState.filter((item) => item !== label));
    }
  };
          ...
          <div className="px-5 pt-5">
            ...
            {roomTypes.map((type, index) => (
              <CheckBox
                ...
                onChange={(e, checked, label) => handleCheckboxChange(e, checked, label, setSelectedRoomTypes, selectedRoomTypes)}
              />
            ))}
            ...
            {priceRange.map((range, index) => (
              <CheckBox
                ...
                onChange={(e, checked, label) => handleCheckboxChange(e, checked, label, setSelectedPrices, selectedPrices)}
              />
            ))}
